...
<p align="center">
  <strong>你的 coding agent 說「我做完了」的時候，它是拿哪把尺量的？<br />
  Witnos 把那把尺攤出來——而且讓你在它還在跑的時候就改。</strong>
</p>
...
<p align="center">
  <a href="#安裝與執行">安裝</a> ·
  <a href="#三十秒版本">三十秒版本</a> ·
  <a href="#它實際做了什麼">功能</a> ·
  <a href="#設計原則這些是這個專案的真正內容不是-ui">設計原則</a> ·
  <a href="#粗略-roadmap">Roadmap</a>
</p>
...
只要「怎麼算做到了」還有解讀空間，agent 就會拿它自己那套尺自我驗證、然後回報「完成」——而那不一定是你要的。跑得越久，第一步沒抓到的偏差就越是變成後面所有東西的地基。

Witnos 是一層薄薄的東西，掛在你既有的 agent（目前是 Claude Code）上，做三件事：

1. **逼 agent 把驗證攤開** —— 每條驗收條件寫成「主張 ＋ 怎麼驗 ＋ 我判定完成時看到的證據」，證據必附出處（檔案、指令、URL），你一鍵打開原物。
2. **讓你在它還在跑的時候改那把尺** —— 改動當場經由 hook 送進 agent 的對話裡，不必打斷它、也不必等它收工。
3. **攔下「我做完了」** —— agent 想停時，Stop hook 對照契約最新版；沒對齊就擋回去，逼它處理完再停。

它**不是** agent，也不取代你的 agent。你自帶 agent 與憑證（BYOK），Witnos 只負責讓驗證那一步變得看得見、能介入。

名字取自 **witness**（見證）——agent 做的每件事都在你眼皮底下被看著。

> **這份 README 同時是本專案的設計文件。** 上半部是給想跑跑看的人，〈設計原則〉以下是完整的設計脈絡與待驗證假設——那才是這個專案的真正內容。
...
**📋 活契約，不是事前規格。** 驗收清單在 agent 執行的全程都可編輯。你改了一條，contract version 就往前推一格；agent 下一次呼叫工具之後，變動的那幾條會被注入它的對話——**永遠只送 delta**，不是整份清單，否則它會回頭翻案已經做完的項目、token 也燒不完。

**🔒 「我做完了」會被攔下來，而且是 fail closed。** Stop hook 在 agent 想收工時對照契約最新版：客觀項全過、主觀項都攤出詮釋與證據、已對齊最新版本，才放行。連不上 Witnos 的時候**擋住而不是放行**——整個產品的價值就是那個攔停，所以它不能因為服務掛了就消失。（只有被監看的專案會這樣，其他專案完全不受影響。）

**🔍 證據必附出處。** 每條證據都要帶檔案路徑、跑過的指令或 URL，UI 一鍵打開原物。工具還會記錄「你打開原物之後、改了哪條尺」——那是防止裁判淪為橡皮圖章的訊號，也是本專案核心假設的直接讀數。

**⚖️ 主觀項目 agent 不准自己宣告通過。** 分類預設一律主觀；只有掛著機器可執行 oracle（一條指令＋預期輸出）的才算客觀。分類錯誤的方向永遠往「多給人看」倒。

**🖥 終端機活得比 app 久。** agent 跑在 app 內建的終端機裡，而那些 shell 住在一支獨立的 daemon——關掉 Witnos 再打開，session 連同對話都還在。（Windows 除外，見限制。）

**↩︎ 你的修正能打回還在跑的 agent。** 事後裁判對一個已經停下的 agent 是沒有意義的，所以 Witnos 會把改動直接打進 agent 所在的那個終端機——而且會先讀 Claude Code 自己的標題狀態，它在忙就不打擾（反正送信通道下一次工具呼叫就會送到）。

**🔑 BYOK。** 不碰你的 token、不代理憑證。全部 local-first，沒有雲端、不收集任何資料。
...
- **終端機活得比 app 久（2026-07-30，已端到端確認）。** goal 綁一個 session id，而那個 id 永不復生 —— 所以 GUI 一關就把 shell 掛斷，等於一份契約的壽命只有一次 app 執行：辛苦長出來的驗證項在下次開啟時對 agent 已經是死的。改由 `witnos pty-serve` 持有那些 shell，也就是 tmux 唯一承重的那個把戲，但不把 tmux 變成依賴（承重路徑維持單一語言、單一 repo，而那支 headless bin 本來就不准依賴 `tauri`）。協議的完整規格住在該模組的檔頭；形狀是**一個 socket、兩種連線**：控制連線走 newline-delimited JSON（open／list／resize／kill／**foreground**），資料連線一個 session 一條、hello 之後就是**不帶任何 framing 的裸位元流** —— agent 印幾 MB 的時候，不該為 base64 或每塊表頭付錢。attach 時先回放有界的 scrollback，再無縫接上即時輸出；那個接縫由結構保證，不是靠時序湊。session id 跨 daemon 重啟單調遞增且永不重用：id 就是 goal 用來指名「我的 agent 住在哪個終端機」的東西，重發一個還活著的 id 等於把修正打給錯的 agent。
  ...
  **一個 pane 屬於一個專案；關掉目標就開下一個 shell（2026-07-30）。** shell 一旦持久化，pane 就不再只是一個輸入框，而是一個工作區——裡面住著一個 agent、一份逐字紀錄、一個所在目錄。所以在側欄點選專案不再把你正在打字的那個 shell `cd` 過去，而是在該專案開一個 pane、原本那個留在原地；點回來時會找到已經在那個專案裡的 pane（或你 cd 進某個 crate 之後仍在它底下的那個）給焦點，而不是再開第二個——就像在編輯器裡切換專案。`cd` 那條路只留作最後手段：疊到沒有空間再開 pane 時，選擇是「搬一個 shell 過去」或「這次點擊什麼都沒發生」，前者比較誠實（而且它依然拒絕對正在跑東西的 pane 打字）。關掉目標同理，**在它的專案裡開一個新的 shell**：goal 綁一個 session id、id 永不復生，所以下一次下目標本來就需要新的 session，工作區直接把它遞過來，而不是留成一個要記得自己做的步驟；被關掉的那個目標自己的 pane 完全不動——那裡是剛完成的工作的逐字紀錄，而「只有 ✕ 與 restart 會殺掉 shell」這條不破。兩者都要等啟動 census：census 之前 app 分不出「這個專案沒有 shell」與「還不知道有哪些 shell」，會開出一個自己正要還原的 pane 的複本——與上面殭屍記帳同一條規則：「我不知道」不是答案。
...
| 路徑 | 是什麼 |
|---|---|
| `crates/witnos-core` | 領域型別、寫入時的規則、一目標一檔的 JSON store、守門的放行條件。不依賴任何 I/O 框架 |
| `crates/witnos-server` | axum 核心，**以 lib 形式**存在（Tauri 外殼內嵌它，所以人改的與守門讀的是同一份 in-process store）。`examples/serve.rs` 可以無頭跑 |
| `crates/witnos-cli` | headless bin `witnos`：兩個 hook（Stop 守門 fail-closed／PostToolUse 送信 fail-open）、上膛／退膛、agent 用的契約子命令、`pty-serve` 終端機 daemon。**不得依賴 `tauri` crate** |
| `crates/witnos-app` | Tauri 外殼。人的那一側走 IPC，agent 那一側走 HTTP——這個分界是結構性的信任邊界 |
| `ui/` | webview 前端（React + TypeScript，無 UI 框架依賴，圖示是內嵌的 Lucide path） |
| `scripts/install-app.sh` | 一鍵打包安裝成 `/Applications/Witnos.app` |

開發指令（測試、lint、各種 build 迴圈）在 `CLAUDE.md`。

---

## 參與

還在自用驗證階段，歡迎 issue 與 PR，但先讀一下〈設計原則〉——那六條是硬約束，任何實作決定都要對得上它們。尤其是原則 2（主觀項目 agent 不得自己過關）與 fail-closed 守門，那兩條是這個工具存在的理由，不接受為了方便而放寬。

覺得方向對但細節想換一套做法？這專案刻意不追求護城河，**乾淨到容易看懂、容易 fork 是它的功能之一**——直接 fork 走也完全沒問題。
